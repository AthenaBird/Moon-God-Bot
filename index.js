...
client.once("ready", () => {
  ...
  if (!emojis_sql["count(*)"]) {
    ...
    sql_emojis
      ...
      .run();
    // Ensure that the "id" row is always unique and indexed.
    sql_emojis.prepare("CREATE UNIQUE INDEX idx_emojis_id ON emojis (id);").run();
    sql_emojis.pragma("synchronous = 1");
    sql_emojis.pragma("journal_mode = wal");
  }
  ...
  // birthdays SQL
  const birthdays_sql = sql_birthdays
    .prepare(
      "SELECT count(*) FROM sqlite_master WHERE type='table' AND name = 'birthdays';"
    )
    .get();
  if (!birthdays_sql["count(*)"]) {
    // If the table isn't there, create it and setup the database correctly.
    sql_birthdays
      .prepare(
        "CREATE TABLE birthdays (id TEXT PRIMARY KEY, name TEXT, month INT, day INT);"
      )
      .run();
    // Ensure that the "id" row is always unique and indexed.
    sql_birthdays.prepare("CREATE UNIQUE INDEX idx_birthdays_id ON birthdays (id);").run();
    sql_birthdays.pragma("synchronous = 1");
    sql_birthdays.pragma("journal_mode = wal");
  }
...
client.on("message", message => {
  ...
  //AC BOT LINK
  if (message.author.id == "715045349114380349") {
    const guild = client.guilds.get("425866519650631680");
    var bot_thumbnail = guild.member(message.author.id);
    var channel_id = message.content.substr(2, message.content.indexOf(" ") - 2)
    var message_content = message.content.substr(message.content.indexOf(" ") + 1);
    
    console.log(channel_id);
    
    if(message_content.startsWith("!!")) {
       client.channels.get("720835094079733800").send("Alpha Centuari is confused by your input: \n> Error message: **" + message_content.substr(2) + "**");
       
    } else {    
    
    var embed_AC = new Discord.RichEmbed()
      .setTitle("**🤔💬🤔**")
      .setDescription("*Predictive text generated by <@715045349114380349>.*")
      .setColor("0x3272d9")
      .setThumbnail(bot_thumbnail.user.displayAvatarURL)
      .addField("\u200b", "> " + message_content, false)
      .setFooter("Report bugs, or ideas, to Jason | Data Sci | Warren !")
    
    
      client.channels.get(channel_id).send(embed_AC);
  ...
  if(message.content.toLowerCase().includes("ucb")) {
		  message.channel.send('Recently a meme was shared to our subeddit outlining a dismissive and disrespectful act that, often, many non-Berkeley students are guilty of. \n \n' + 
                           ...
                           'TL:DR Don\'t say \"UCB\" when referring to Berkeley.');
  ...
  try {
    ...
    } else if (commandName === "generate" || commandName === "complete") {
      command.execute(message, args, client);  
